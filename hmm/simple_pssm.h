...
Copyright John Reid 2007, 2013, 2015
...
calculate_log_scores_for_nmer( const DistRange & dist, Scores & scores )
{
    ...
    for( int i = 0; size(scores) != size_t( i ); ++i ) {
        if( 4 != size( const_cast< DistRange & >( dist )[i]) ) throw std::runtime_error( "dist must have elements with length 4" );
        if( 5 != size( const_cast< Scores & >( scores )[i]) ) throw std::runtime_error( "scores must have elements with length 5" );
...
calculate_complementary_scores( const Scores & scores, Scores & complementary_scores )
{
    ...
    for( unsigned i = 0; len != i; ++i ) 
    {
        ...
        if( size( const_cast< Scores & >( scores )[i] ) != 5 ) throw std::runtime_error( "Scores should have entries of length 5." );
